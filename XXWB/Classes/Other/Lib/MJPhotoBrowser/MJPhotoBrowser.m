...
- (void)loadImageNearIndex:(int)index
{
    if (index > 0) {
        ...
        [self.manager downloadImageWithURL:photo.url options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
...
//        [SDWebImageManager downloadWithURL:photo.url];
    ...
    if (index < _photos.count - 1) {
        MJPhoto *photo = _photos[index + 1];
        [self.manager downloadImageWithURL:photo.url
                                   options:0
